...
<section style="padding-top:5.5rem;background:linear-gradient(135deg,#00074B 0%,#050e3a 60%,#0b1854 100%);color:#fff;padding-bottom:2.5rem;">
  <div class="container" style="max-width:800px;">
    ...
    <h1 style="font-size:2.4rem;font-weight:800;line-height:1.25;">{{ $post->title }}</h1>

    @if($post->excerpt)
      <p style="font-size:1.1rem;opacity:0.65;line-height:1.6;margin-top:1rem;">{{ $post->excerpt }}</p>
    @endif
  </div>
</section>
...
<section class="section" style="padding-top:{{ $post->image ? '2.5rem' : '0' }};">
  <div class="container" style="max-width:780px;">
    ...
    {{-- Share --}}
    <div style="margin-top:2.5rem;padding-top:1.5rem;border-top:1px solid var(--border);display:flex;align-items:center;gap:1rem;">
      <span style="font-size:0.85rem;color:var(--gray-400);font-weight:600;">Поделиться:</span>
...
<section class="section section-gray" style="padding-top:2.5rem;">
  <div class="container">
    <h2 style="font-size:1.3rem;font-weight:700;margin-bottom:1.5rem;color:var(--dark);">
      {{ $post->type === 'news' ? 'Другие новости' : 'Другие статьи' }}
    </h2>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:1.25rem;">
      @foreach($recent as $r)
        <a href="{{ route('post.show', $r) }}" class="related-post reveal">
          ...
          <div class="related-post-body">
            ...
            <h3 style="font-size:0.95rem;font-weight:700;color:var(--dark);line-height:1.35;">{{ $r->title }}</h3>
          </div>
        </a>
      @endforeach
    </div>
...
<style>
  .post-article {
    font-size:1.08rem;line-height:2;color:var(--dark);
  }
